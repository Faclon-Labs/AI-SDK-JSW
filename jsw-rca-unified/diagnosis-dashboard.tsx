...
// xlsx replaced with exceljs (no known CVEs)
...
const getHighPowerSubsections = (millType: string) => {
  ...
  // Excel export function — uses exceljs (replaces vulnerable xlsx package)
  const exportToExcel = async () => {
    try {
      const ExcelJS = (await import('exceljs')).default;
      const workbook = new ExcelJS.Workbook();

      // Helper: add a sheet from an array-of-arrays and return the worksheet
      ...
      const styleCell = (cell: any, bold: boolean, fontArgb: string, fillArgb: string) => {
        cell.font = { bold, color: { argb: fontArgb } };
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fillArgb } };
      };

      // Helper: apply styling and auto-fit to a worksheet
      const applyStylesAndAutoFit = (ws: any) => {
        const colWidths: number[] = [];

        ws.eachRow((row: any, rowIdx: number) => {
          row.eachCell({ includeEmpty: false }, (cell: any, colIdx: number) => {
            const v = cell.value;

            if (rowIdx === 1) {
              // First row header — blue background
              styleCell(cell, true, 'FFFFFFFF', 'FF3498DB');
            } else if (
              colIdx === 1 &&
              typeof v === 'string' &&
              (v.includes('Analysis') || v.includes('Information') ||
                v.startsWith('Date:') || v.includes('Summary'))
            ) {
              styleCell(cell, true, 'FF2C3E50', 'FFECF0F1');
            } else if (typeof v === 'string') {
              const lower = v.toLowerCase();
              if (lower === 'high') styleCell(cell, true, 'FFE74C3C', 'FFE74C3C20'.slice(0, 10));
              else if (lower === 'medium') styleCell(cell, true, 'FFF39C12', 'FFF39C1220'.slice(0, 10));
              else if (lower === 'normal') styleCell(cell, true, 'FF27AE60', 'FF27AE6020'.slice(0, 10));
              else if (lower === 'low') styleCell(cell, true, 'FF34495E', 'FF34495E20'.slice(0, 10));
            }

            // Track max content width per column
            const len = v != null ? String(v).length : 0;
            colWidths[colIdx - 1] = Math.min(Math.max(colWidths[colIdx - 1] ?? 10, len + 2), 50);
          });
        });
      ...
      applyStylesAndAutoFit(addSheet('Executive Summary', summaryData));

      // Diagnostic Data sheet
      const diagnosticHeaders = ['MILL NAME', 'TARGET', 'Day SPC', 'Deviation', 'Impact', 'Day'];
      ...
      applyStylesAndAutoFit(addSheet('Diagnostic Data', [diagnosticHeaders, ...diagnosticRows]));

      // TPH Analysis sheet
      const tphHeaders = ['Asset Name', 'Timestamp', 'Device', 'Target TPH', 'Cause', 'Both RP Down Duration', 'Both RP Down Ramp-up', 'Reduced Feed Duration', 'Ball Mill Duration'];
      const tphRows: any[][] = [];
      finalFilteredData.forEach(item => {
        if (item.backendData?.TPH) {
          ...
          tphRows.push([
            ...
            tph.Device || 'N/A',
            formatNumber(tph.target),
            tph.cause || 'N/A',
            tph.both_rp_down?.[1] || 'N/A',
            tph.both_rp_down?.[2] || 'N/A',
            tph["Reduced Feed Operations"]?.[1] || 'N/A',
            tph.ball_mill?.[1] || 'N/A',
          ]);
        }
      });
      if (tphRows.length > 0) applyStylesAndAutoFit(addSheet('TPH Analysis', [tphHeaders, ...tphRows]));

      // High Power Analysis sheet
      const highPowerHeaders = ['Asset Name', 'Timestamp', 'Device', 'Subsection', 'Cause', 'Sensor Info'];
      const highPowerRows: any[][] = [];
      finalFilteredData.forEach(item => {
        if (item.backendData?.High_Power) {
          const hp = item.backendData.High_Power as any;
          Object.entries(hp).forEach(([subsection, data]) => {
            if (typeof data === 'object' && data !== null && subsection !== 'Device') {
              ...
              highPowerRows.push([
                ...
                subsection,
                d.cause || 'N/A',
                d.sensor ? JSON.stringify(d.sensor) : 'N/A',
              ]);
            }
          });
        }
      });
      if (highPowerRows.length > 0) applyStylesAndAutoFit(addSheet('High Power Analysis', [highPowerHeaders, ...highPowerRows]));

      // Maintenance Events sheet
      const maintenanceHeaders = ['Asset Name', 'Equipment', 'Start Date Time', 'End Date Time', 'Event Details', 'Department', 'Stoppage Category', 'Reason of Stoppage', 'Duration'];
      const maintenanceRows: any[][] = [];
      finalFilteredData.forEach(item => {
        if (item.backendData?.TPH) {
          const tph = item.backendData.TPH as any;
          const pushEvents = (events: any[], equipment: string) => {
          ...
          pushEvents(tph.RP1_maintance, 'RP1');
          pushEvents(tph.RP2_maintance, 'RP2');
        }
      });
      if (maintenanceRows.length > 0) applyStylesAndAutoFit(addSheet('Maintenance Events', [maintenanceHeaders, ...maintenanceRows]));
                                      ...
                                      {item?.backendData?.TPH?.["Reduced Feed Operations"] && (
                                        <div className="bg-gray-50 rounded-lg p-4">
                                          <div className="flex items-center justify-between mb-3">
                                            ...
                                            <button
                                              onClick={() => openPlusPopup(
                                                "Reduced Feed Operations",
                                                item?.sectionName || "",
                                                "TPH.Reduced Feed Operations",
                                                item?._id || "",
                                                item?.insightID || "",
                                                item?.applicationType || "Workbench",
                                                item?.backendData || {}
